...
export class VimWorker {
    ...
    constructor(scriptPath: string, onMessage: (msg: MessageFromWorker) => void, onError: (error: Error) => void) {
    ...
    async notifyEvalFuncError(msg: string, error: Error, dontReply: boolean) {
        const errmsg = `${ msg } for jsevalfunc(): ${ error.message }: ${ error.stack }`;
        if (dontReply) {
        ...
        debug("Sent exception thrown by evaluated JS function:", msg, error);
    }
    ...
    private recvMessage(event_: MessageEvent) {
        const msg: MessageFromWorker = event_.data;
    ...
    private recvError(event_: ErrorEvent) {
        debug("Received an error from worker:", event_);
        const msg = `${ event_.message } (${ event_.filename }:${ event_.lineno }:${ event_.colno })`;
...
export class VimWasm {
    ...
    private async evalJS(path: string, contents: ArrayBuffer) {
        ...
        try {
            ...
            debug("Failed to evaluate", path, "with error:", error);
            // @ts-expect-error vendor
            ...
            case "read-clipboard:request":
                if (this.readClipboard) {
                    this.readClipboard()
                        ...
                        .catch(error => {
                            debug("Cannot read clipboard:", error);
    ...
    private printPerfs() {
...

        {
            ...
            for (const event_ of performance.getEntries()) {
                const ms = measurements.get(event_.name);
                if (ms === undefined) {
                    measurements.set(event_.name, [event_]);
                }
                else {
                    ms.push(event_);
                }
